...
//#define DEBUG_MIRRORZINHO_PUZZLE
...
MirrorzinhoPuzzle::MirrorzinhoPuzzle(GameObject& associated, std::vector<Piece> pieces) : Component(associated),
...
int MirrorzinhoPuzzle::ClosestPiece(Vec2 joystick) {
...
void MirrorzinhoPuzzle::Update(float dt) {
    if (solved) return;
...
void MirrorzinhoPuzzle::RenderPiece(int i) {
...
void MirrorzinhoPuzzle::Start() {
    ...
    pieces[5].pos = {(float) ((randomOffset) - 325), 525};
}

bool MirrorzinhoPuzzle::IsSolved() {
    for (int i = 0; i < pieces.size(); i++) {
...
bool MirrorzinhoPuzzle::Is(std::string type) {
    return type == "MirrorzinhoPuzzle";
}

// MirrorPuzzle initiator
MirrorzinhoPuzzle::Initiator::Initiator(GameObject& associated) : Component(associated) {
...
void MirrorzinhoPuzzle::Initiator::Update(float dt) {
    ...
    if (GameData::mirrorzinhoPuzzleSolved) {
        ...
        intr->SetAction([this, intr](State* state, GameObject* go) {
            ...
            mp->AddComponent(new MirrorzinhoPuzzle((*mp), std::vector<Piece>{
...
void MirrorzinhoPuzzle::Initiator::Render() {}

void MirrorzinhoPuzzle::Initiator::Start() {}

bool MirrorzinhoPuzzle::Initiator::Is(std::string type) {
    return type == "MirrorzinhoPuzzleInitiator";
}
